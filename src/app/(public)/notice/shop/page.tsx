      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((product) => (
            <div
              key={product.id}
              className="rounded-2xl bg-[#111d35] border border-white/10 overflow-hidden hover:border-[#c9a84c]/30 transition-all duration-300 group"
            >
              {/* Placeholder Image */}
              ...
              {/* Product Info */}
              <div className="p-5">
                <h3 className="text-white font-medium text-sm leading-snug line-clamp-2 mb-3 min-h-[2.5rem]">
                  {product.name}
                </h3>
                ...
                <button className="w-full py-2.5 rounded-lg bg-[#c9a84c] text-[#0a1628] font-semibold text-sm hover:bg-[#d4b85c] transition-colors">
                  구매하기
                </button>
              </div>
            </div>
