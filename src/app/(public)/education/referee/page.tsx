...
const levels = [
  {
    level: "3급 심판",
    description: "시/도 단위 대회 심판 자격",
    duration: "60시간 (3주)",
    requirement: "만 20세 이상, 관련 분야 기본 지식 보유자",
    fee: "400,000원",
  },
  {
    level: "2급 심판",
    description: "전국 단위 대회 심판 자격",
    duration: "80시간 (4주)",
    requirement: "3급 심판 자격 취득 후 1년 이상 경력자",
    fee: "550,000원",
  },
  {
    level: "1급 심판",
    description: "국제 대회 심판 자격",
    duration: "100시간 (5주)",
    requirement: "2급 심판 자격 취득 후 2년 이상 경력자",
    fee: "700,000원",
  },
];
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {levels.map((item, index) => (
              <div
                key={item.level}
                className={`rounded-xl border bg-[#111d35]/80 backdrop-blur-sm p-6 transition-all duration-300 hover:border-[#c9a84c]/40 ${
                  index === 2
                    ? "border-[#c9a84c]/30 ring-1 ring-[#c9a84c]/10"
                    : "border-white/10"
                }`}
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  {index === 2 && (
                    <span className="text-xs px-2 py-0.5 rounded bg-[#c9a84c]/20 text-[#c9a84c] font-semibold">
                      최상위
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-400">교육시간</span>
                    <span className="text-white">{item.duration}</span>
                  </div>
                  <div className="w-full h-px bg-white/5" />
                  <div className="flex justify-between">
                    <span className="text-gray-400">교육비</span>
                    <span className="text-white font-semibold">{item.fee}</span>
                  </div>
                  <div className="w-full h-px bg-white/5" />
                  <div>
                    <span className="text-gray-400">지원자격</span>
                    <p className="text-gray-300 mt-1">{item.requirement}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="rounded-xl border border-white/10 overflow-hidden bg-[#111d35]/80 backdrop-blur-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#1a2744]">
                    <th className="px-6 py-4 text-center text-[#c9a84c] font-semibold">과정</th>
                    <th className="px-6 py-4 text-center text-[#c9a84c] font-semibold">등급</th>
